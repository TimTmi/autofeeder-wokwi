...
void feedToTargetPortion()
{
    ...
    float current_weight = weightSensor.getWeight();
    float target_weight = weightSensor.getTargetPortion();
    ...
    if (ok)
    {
        mqttManager.publish("feed_started", String(target_weight), false);
    }
    else
    {
...
void loop()
{
    ...
    if (now - lastStatusPublish >= 5000)
    {
        ...
        Serial.print("[MQTT] Storage: ");
        Serial.print(storagePercent);
        Serial.print("%, ");
        Serial.print(storageWeight);
        Serial.print("g, ");
        Serial.println(storageState);
    }
